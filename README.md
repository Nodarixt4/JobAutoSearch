...
O fluxo usa exclusivamente o mesmo Gemini, por padrão `gemini-3.6-flash`, em duas etapas: pesquisa em linguagem natural com `google_search`, sem exigir JSON; depois extração estruturada sem ferramentas, com esquema JSON. A extração recebe texto, fontes e `groundingSupports` da pesquisa, sem partes de raciocínio. A resposta deve conter exatamente `titulo`, `empresa`, `local`, `descricao` e `url`, todos strings não vazias. O backend valida formato, limites de tamanho e URLs HTTP(S) sem credenciais embutidas. O raciocínio mantém nível médio nos modelos Gemini 3. Não há fallback de modelo ou provedor.
...
As URLs dos cartões devem pertencer às fontes de grounding da pesquisa original; URLs inventadas ou introduzidas apenas na extração são rejeitadas. Redirecionamentos Google são preservados, sem tentar adivinhar o destino. `groundingMetadata`, `researchText` e as sugestões Google originais são preservados; os índices dos metadados se referem ao texto de pesquisa, não ao JSON extraído. A correspondência de URL não comprova a correção semântica de todos os campos, inspeção integral da página ou candidatura aberta. A aplicação não realiza verificação independente dos anúncios.

Há no máximo duas chamadas de pesquisa (retry apenas por ausência de grounding) e uma extração, todas no mesmo prazo total de 110 segundos. A extração não é repetida. Pesquisa sem texto, extração interrompida ou link fora das fontes falham sem virar lista vazia. A extração adiciona custo de tokens; cada chamada de pesquisa pode executar várias consultas faturáveis, portanto o limite de chamadas não é um teto financeiro.

A documentação oficial de [Google Search em generateContent](https://ai.google.dev/gemini-api/docs/generate-content/google-search) informa que o modelo decide quando pesquisar. A de [saída estruturada](https://ai.google.dev/gemini-api/docs/generate-content/structured-output) permite combinar ferramentas e JSON no Gemini 3. Logo, exigir JSON no prompt antigo é uma hipótese de interferência, não uma causa comprovada de `MISSING_GROUNDING`. A separação remove essa exigência da pesquisa, mas não garante que o provedor sempre retorne grounding. Falhas persistentes continuam sendo rejeitadas.
