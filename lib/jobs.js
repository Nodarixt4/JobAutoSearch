...
function matches(item, query) {
  ...
  if (query.modality !== 'all' && item.modality && item.modality !== query.modality) return false;
  const levels = {
...
export async function aggregateJobs({ query = normalizeQuery(), apiKey, model, fetchImpl = fetch, gemini = searchJobs, allowProvider = () => true, feeds = new Map(), now = Date.now } = {}) {
  ...
  const results = await Promise.allSettled(query.providers.map(async id => {
    ...
    if (!entry || now() - entry.time >= 15 * 60000) {
      ...
      entry = { time: now(), promise: Promise.resolve().then(async () => {
        const data = await readJson(await fetchImpl(url, { signal: AbortSignal.timeout(15000), headers: { Accept: 'application/json' } }));
        if (!Array.isArray(id === 'remotive' ? data.jobs : data.data)) throw new SearchError('Formato inesperado do provedor.');
        return data;
      }).catch(error => { feeds.delete(id); throw error; }) };
      feeds.set(id, entry);
    }
    const data = await entry.promise;
    const rows = id === 'remotive' ? data.jobs : data.data;
    ...
    const items = rows.slice(0, 5000).flatMap(row => {
      ...
      const local = plain(row.candidate_required_location || row.location) || 'Local nao informado';
      ...
      modality: remote ? 'remote' : modality(`${row.title} ${row.description}`) }];
    });
    return { items, truncated: rows.length > 5000 || Boolean(data.links?.next), sources: [{ title: provider.label, url }], searchEntryPoint: '' };
  }));
  ...
  results.forEach((result, index) => {
    ...
    providers.push({ id, label, status: 'ok', count: filtered.length, message: 'Cobertura limitada; modalidade e senioridade desconhecidas sao mantidas. Local e termos exigem correspondencia textual.' });
  });
  ...
  return { ...extra, jobs: jobs.slice(0, 40), sources, searchEntryPoint, searchedAt: new Date().toISOString(), providers, truncated: truncated || jobs.length > 40 };
}