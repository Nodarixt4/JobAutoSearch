...
export function parseSearch(payload) {
  ...
  if (!Array.isArray(value) || value.length > 10) throw new SearchError('Lista de vagas invalida.');
...
Retorne SOMENTE array JSON, ate 10 vagas, sem markdown. Cada objeto deve ter exatamente titulo, empresa, local, descricao, url (strings nao vazias). Descricao fiel em ate 300 caracteres. Local: Cidade/MT - Presencial ou Cidade/MT - Hibrido ou Remoto - Brasil. URL HTTP(S) individual encontrada na pesquisa. Nao invente links nem vagas. Remova duplicatas. Se a pesquisa foi feita mas nao encontrou anuncios adequados, retorne [].`;
  try {
    ...
    const response = await fetchImpl(`https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(model)}:generateContent`, {
      ...
      body: JSON.stringify({
        ...
        tools: [{ google_search: {} }],
        generationConfig: { temperature: 0.2, maxOutputTokens: 4096, ...(thinkingConfig && { thinkingConfig }) }
      })
    });
    if (!response.ok) {
