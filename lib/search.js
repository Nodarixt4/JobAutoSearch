...
export async function searchJobs({ apiKey, model = 'gemini-3.6-flash', fetchImpl = fetch, logger = console, query = {} }) {
  ...
  const prompt = `Pesquise agora no Google vagas reais. Data: ${new Date().toISOString()}.
...
Responda em linguagem natural com ate 40 anuncios e citacoes das fontes. Para cada anuncio, descreva titulo, empresa, local, atividades e link individual encontrado. Nao invente links nem vagas. Remova duplicatas. Se nao encontrar anuncios adequados, explique isso e as limitacoes da pesquisa.`;
  try {
    ...
    const extracted = await request({
      ...
      contents: [{ role: 'user', parts: [{ text: JSON.stringify({ query, geographicPolicy: 'Remoto explicitamente Brasil/Brazil, America Latina/Latin America/LATAM ou worldwide; presencial/hibrido somente Mato Grosso MT, nunca MS. Excluir desconhecidos. Preservar evidencia de elegibilidade e modalidade no campo local.', research: text, sources, groundingSupports: metadata.groundingSupports || [] }) }] }],
      generationConfig: {
