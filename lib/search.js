...
export function parseSearch(payload) {
  ...
  if (candidate?.finishReason !== 'STOP') {
    throw new SearchError('O Gemini bloqueou ou interrompeu a resposta. Tente novamente mais tarde.');
  }
  ...
  if (!Array.isArray(value) || value.length > 18) throw new SearchError('Lista de vagas invalida.');
...
export async function searchJobs({ apiKey, model = 'gemini-2.5-flash', fetchImpl = fetch }) {
...
Retorne SOMENTE array JSON, ate 18 vagas, sem markdown. Cada objeto deve ter exatamente titulo, empresa, local, descricao, url (strings nao vazias). Descricao fiel em ate 450 caracteres. Local: Cidade/MT - Presencial ou Cidade/MT - Hibrido ou Remoto - Brasil. URL HTTP(S) individual encontrada na pesquisa. Nao invente links nem vagas. Remova duplicatas. Se a pesquisa foi feita mas nao encontrou anuncios adequados, retorne [].`;
  try {
